...
describe("Loop 3 — App Generation Department (quick validation only, no full OS execution)", () => {
  it("has exactly 10 agent specs", () => {
    expect(APP_GENERATION_DEPARTMENT).toHaveLength(10);
  });
  ...
  it("Registry verification: all 10 agents have generated .claude/agents/ files", () => {
  ...
  it("Registry verification: all 10 agents appear in registry/agents.registry.json with status active", () => {
  ...
  it("Artifact verification: all 10 agents have the full per-agent scaffold folder", () => {
    for (const name of APP_GENERATION_DEPARTMENT_AGENTS) {
  ...
  it("Runtime verification: department wires into AgentRuntime and all 10 agents become active", () => {
  ...
  it("Runtime discovery: solution-architect-app is discoverable by tag and category", () => {
    ...
    expect(byTag.length).toBeGreaterThanOrEqual(10);
