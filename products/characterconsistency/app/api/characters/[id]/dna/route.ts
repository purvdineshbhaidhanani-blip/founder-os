...
import { consumeAiCredit } from "@founder-os/platform/billing";
...
export async function POST(_request: Request, { params }: RouteParams) {
  return withRouteHandler(async () => {
    ...
    if (!(await can(organizationId, "use_story_memory"))) {
      throw new PlatformError("UNAUTHORIZED", "AI Character DNA requires the Starter plan or higher.");
    }
    const { id } = await params;
    return consumeAiCredit(organizationId, () =>
      generateCharacterDNA({ organizationId, characterId: id, requestedByUserId: userId }),
    );
  });
}
