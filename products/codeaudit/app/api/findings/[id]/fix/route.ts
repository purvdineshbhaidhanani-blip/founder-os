...
export async function POST(_request: Request, { params }: RouteParams) {
  return withRouteHandler(async () => {
    ...
    if (!(await can(organizationId, "use_ai_fix_engine"))) {
      throw new PlatformError("UNAUTHORIZED", "The AI Fix Engine requires the Starter plan or higher.");
    }
    const { id } = await params;
    return consumeAiCredit(organizationId, () =>
      generateFixSuggestion({ organizationId, findingId: id, requestedByUserId: userId }),
    );
  });
}
