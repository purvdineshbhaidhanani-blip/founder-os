...
export async function POST(_request: Request, { params }: RouteParams) {
  return withRouteHandler(async () => {
    ...
    if (!(await can(organizationId, "use_ai_summary"))) {
      throw new PlatformError("UNAUTHORIZED", "AI audit summaries require an active plan.");
    }
    const { id } = await params;
    return generateAuditSummary({ organizationId, findingId: id, requestedByUserId: userId });
  });
}
