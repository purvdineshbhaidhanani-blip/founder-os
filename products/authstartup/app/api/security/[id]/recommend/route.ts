...
export async function POST(_request: Request, { params }: RouteParams) {
  return withRouteHandler(async () => {
    ...
    if (!(await can(organizationId, "use_ai_security_advisor"))) {
      throw new PlatformError("UNAUTHORIZED", "The AI Security Advisor requires the Starter plan or higher.");
    }
    const { id } = await params;
    return consumeAiCredit(organizationId, () =>
      generateSecurityRecommendation({ organizationId, findingId: id, requestedByUserId: userId }),
    );
  });
}
