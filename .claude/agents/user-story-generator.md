...
description: Converts features into structured user stories for development teams.
...
> Converts features into structured user stories for development teams.

- **Category:** engineering
- **Owner:** engineering-department
- **Tags:** engineering-department, product, discovery, stories
...
- **Input format:** Receives work from mvp-planning-agent via the Master Orchestrator's task queue, with task context loaded by the Context Manager.
- **Output format:** Delivers results to success-metrics-agent; artifacts are published through the Artifact Manager and recorded in shared memory, then announced on the Event Bus.
- **Escalation path:** Reports to mvp-planning-agent. On failure, emits a failure event on the Event Bus; the Master Orchestrator applies the task-queue retry policy and routes any human-gated step through the Approval System before resuming the workflow.
- **Collaborates with:** mvp-planning-agent, success-metrics-agent
...
- **Blueprint name:** user-story-generator
- **Blueprint content hash:** 83c4c6bc647cdabf
- **Generated at:** 2026-07-01T07:06:06.627Z
