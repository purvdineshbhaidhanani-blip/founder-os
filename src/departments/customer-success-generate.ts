...
import { CUSTOMER_SUCCESS_DEPARTMENT } from "./customer-success.js";

const CUSTOMER_SUCCESS_OWNER = "customer-success-department";

/**
 * Runs the full Agent Factory pipeline for every Customer Success & Support
 * Department spec:
 *
...
async function main(): Promise<void> {
  ...
  process.stdout.write("Customer Success & Support Department\n\n");

  for (const spec of CUSTOMER_SUCCESS_DEPARTMENT) {
    const blueprint = buildDepartmentBlueprint(spec, CUSTOMER_SUCCESS_OWNER);
  ...
  process.stdout.write(
    `\nGenerated ${ok}/${CUSTOMER_SUCCESS_DEPARTMENT.length} agents (${warned} with warnings, ${failed} failed).\n`,
  );
