...
import { FINANCE_LEGAL_DEPARTMENT } from "./finance-legal.js";

const FINANCE_LEGAL_OWNER = "finance-legal-department";

/**
 * Runs the full Agent Factory pipeline for every Finance, Legal & Compliance
 * Department spec:
 *
...
async function main(): Promise<void> {
  ...
  process.stdout.write("Finance, Legal & Compliance Department\n\n");

  for (const spec of FINANCE_LEGAL_DEPARTMENT) {
    const blueprint = buildDepartmentBlueprint(spec, FINANCE_LEGAL_OWNER);
  ...
  process.stdout.write(
    `\nGenerated ${ok}/${FINANCE_LEGAL_DEPARTMENT.length} agents (${warned} with warnings, ${failed} failed).\n`,
  );
