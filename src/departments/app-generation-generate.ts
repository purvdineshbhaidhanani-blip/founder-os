...
import { scaffoldAppGenerationDepartment } from "./app-generation-scaffold.js";
...
async function main(): Promise<void> {
  ...
  process.stdout.write("Loop 3 — App Generation Department\n\n");
  ...
  process.stdout.write(
    `\nGenerated ${ok}/${APP_GENERATION_DEPARTMENT.length} agents (${warned} with warnings, ${failed} failed).\n`,
  );

  if (failed === 0) {
    process.stdout.write("\nScaffolding per-agent folders...\n");
    const scaffolded = await scaffoldAppGenerationDepartment(APP_GENERATION_DEPARTMENT);
