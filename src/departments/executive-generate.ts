...
import { EXECUTIVE_DEPARTMENT } from "./executive.js";

const EXECUTIVE_OWNER = "founder";

/**
 * Runs the full Agent Factory pipeline for every Founder Executive
 * Department spec:
 *
...
async function main(): Promise<void> {
  ...
  process.stdout.write("Founder Executive Department\n\n");

  for (const spec of EXECUTIVE_DEPARTMENT) {
    const blueprint = buildDepartmentBlueprint(spec, EXECUTIVE_OWNER);
  ...
  process.stdout.write(
    `\nGenerated ${ok}/${EXECUTIVE_DEPARTMENT.length} agents (${warned} with warnings, ${failed} failed).\n`,
  );
