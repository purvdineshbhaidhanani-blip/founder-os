...
import { scaffoldFoundationDepartment } from "./foundation-scaffold.js";
...
async function main(): Promise<void> {
  ...
  process.stdout.write(
    `\nGenerated ${ok}/${FOUNDATION_DEPARTMENT.length} agents (${warned} with warnings, ${failed} failed).\n`,
  );

  if (failed === 0) {
    process.stdout.write("\nScaffolding per-agent folder structure...\n");
    const scaffolded = await scaffoldFoundationDepartment(FOUNDATION_DEPARTMENT);
    for (const entry of scaffolded) {
      process.stdout.write(`  ${entry.path}/\n`);
    }
    process.stdout.write(`\nScaffolded ${scaffolded.length} agent folders under agents/\n`);
  }
