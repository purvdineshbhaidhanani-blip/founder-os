...
/**
 * Runs the full Agent Factory pipeline for every Engineering Department spec:
 *
...
async function main(): Promise<void> {
  ...
  for (const spec of ENGINEERING_DEPARTMENT) {
    ...
    process.stdout.write(
      `OK    ${spec.name.padEnd(28)} [${spec.category}]${warnings > 0 ? `  (${warnings} warning${warnings === 1 ? "" : "s"})` : ""}\n`,
    );
  }

  process.stdout.write(
    `\nGenerated ${ok}/${ENGINEERING_DEPARTMENT.length} agents (${warned} with warnings, ${failed} failed).\n`,
  );
