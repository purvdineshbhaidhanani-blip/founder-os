...
import { SALES_MARKETING_DEPARTMENT } from "./sales-marketing.js";

const SALES_MARKETING_OWNER = "sales-marketing-department";

/**
 * Runs the full Agent Factory pipeline for every Sales & Marketing Department
 * spec:
 *
...
async function main(): Promise<void> {
  ...
  process.stdout.write("Sales & Marketing Department\n\n");

  for (const spec of SALES_MARKETING_DEPARTMENT) {
    const blueprint = buildDepartmentBlueprint(spec, SALES_MARKETING_OWNER);
  ...
  process.stdout.write(
    `\nGenerated ${ok}/${SALES_MARKETING_DEPARTMENT.length} agents (${warned} with warnings, ${failed} failed).\n`,
  );
