# Checklist — Requirement Analyzer
...
- [ ] Dependencies (founder, orchestrator-agent) have confirmed their outputs are ready
...
- [ ] Downstream agents (agent-generator, orchestrator-agent) notified via Communication Bus
- [ ] Task outcome written to `agent:requirement-analyzer:last-task` in shared memory
- [ ] Metrics updated in `agent:requirement-analyzer:metrics`
