...
1. **Receive** — Accept task from orchestrator-agent, project-manager via the Master Orchestrator's Task Queue.
2. **Load Context** — Request context load from the Context Manager for this task's namespace.
3. **Validate Input** — Assert input against INPUT_SCHEMA; escalate to project-manager on schema failure.
...
7. **Hand Off** — Notify workflow-engine, orchestrator-agent via the Communication Bus that results are ready.
...
- After 3 failures, escalate to project-manager via the Approval System.
