...
1. **Receive** — Accept task from orchestrator-agent, memory-manager, report-generator via the Master Orchestrator's Task Queue.
...
7. **Hand Off** — Notify orchestrator-agent, decision-engine, context-manager via the Communication Bus that results are ready.
