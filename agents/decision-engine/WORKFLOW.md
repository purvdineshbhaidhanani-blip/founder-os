...
1. **Receive** — Accept task from orchestrator-agent, requirement-analyzer, knowledge-manager via the Master Orchestrator's Task Queue.
...
7. **Hand Off** — Notify orchestrator-agent, knowledge-manager via the Communication Bus that results are ready.
