...
- [ ] Dependencies (orchestrator-agent, requirement-analyzer) have confirmed their outputs are ready
...
- [ ] Downstream agents (agent-registry, quality-controller) notified via Communication Bus
- [ ] Task outcome written to `agent:agent-generator:last-task` in shared memory
- [ ] Metrics updated in `agent:agent-generator:metrics`
