...
- [ ] Dependencies (orchestrator-agent, quality-controller) have confirmed their outputs are ready
...
- [ ] Downstream agents (agent-generator, quality-controller) notified via Communication Bus
- [ ] Task outcome written to `agent:prompt-optimizer:last-task` in shared memory
- [ ] Metrics updated in `agent:prompt-optimizer:metrics`
